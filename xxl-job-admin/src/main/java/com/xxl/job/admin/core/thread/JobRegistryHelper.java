		...
		registryMonitorThread = new Thread(new Runnable() {
			...
			public void run() {
				while (!toStop) {
					try {
						// auto registry group
						List<XxlJobGroup> groupList = XxlJobAdminConfig.getAdminConfig().getGroupService().findByAddressType(0);
						if (groupList!=null && !groupList.isEmpty()) {
							...
							for (XxlJobGroup group: groupList) {
								...
								XxlJobAdminConfig.getAdminConfig().getGroupService().update(group);
							}
						}
					} catch (Exception e) {
						if (!toStop) {
